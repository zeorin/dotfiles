...
let
  ...
  scripts = {
    ...
    setWallpaper = pkgs.writeShellScript "set-wallpaper" ''
      ...
      ${config.wayland.windowManager.hyprland.package}/bin/hyprctl hyprpaper wallpaper "HDMI-A-1,contain:$background_image_left"
      ${config.wayland.windowManager.hyprland.package}/bin/hyprctl hyprpaper wallpaper "DP-1,contain:$background_image_right"
    '';
  };
in
{
  ...
  config = {
    ...
    wayland.windowManager.hyprland = {
      ...
      inherit (moduleArgs.osConfig.programs.hyprland) package portalPackage;
        ...
        bind =
          let
            changegroupactiveormovefocus = pkgs.writeShellScript "changegroupactiveormovefocus" ''
              activewindow="$(${config.wayland.windowManager.hyprland.package}/bin/hyprctl activewindow -j)"
              readonly activewindow
              if ! ${pkgs.jq}/bin/jq -e '.grouped[]' <<< "$activewindow" >/dev/null; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movefocus "$1"
              elif [[ "$1" == l ]] && ${pkgs.jq}/bin/jq -e '.address == .grouped[0]' <<< "$activewindow" >/dev/null; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movefocus l
              elif [[ "$1" == l ]]; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch changegroupactive b
              elif [[ "$1" == r ]] && ${pkgs.jq}/bin/jq -e '.address == .grouped[-1]' <<< "$activewindow" >/dev/null; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movefocus r
              else
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch changegroupactive f
              fi
            '';
            movegroupwindowormovewindoworgroup = pkgs.writeShellScript "movegroupwindowormovewindoworgroup" ''
              ...
              readonly activewindow
              if ! ${pkgs.jq}/bin/jq -e '.grouped[]' <<< "$activewindow" >/dev/null; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movewindoworgroup "$1"
              elif [[ "$1" == l ]] && ${pkgs.jq}/bin/jq -e '.address == .grouped[0]' <<< "$activewindow" >/dev/null; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movewindoworgroup l
              elif [[ "$1" == l ]]; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movegroupwindow b
              elif [[ "$1" == r ]] && ${pkgs.jq}/bin/jq -e '.address == .grouped[-1]' <<< "$activewindow" >/dev/null; then
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movewindoworgroup r
              else
                ${config.wayland.windowManager.hyprland.package}/bin/hyprctl dispatch movegroupwindow f
              fi
            '';
          in
          [
            ...
            "$mod, O, exec, ${config.services.flameshot.package}/bin/flameshot gui --accept-on-select --raw | ${pkgs.imagemagick}/bin/magick -resize 400% png:- png:- | ${pkgs.tesseract}/bin/tesseract -l eng --psm 6 - - | ${pkgs.wl-clipboard}/bin/wl-copy --primary; ${pkgs.libnotify}/bin/notify-send --icon Clipboard --urgency low --expire-time 5000 'OCR result copied to clipboard'"
