...

{
  ...
  options.${namespace}.emacs = {
    enable = lib.options.mkEnableOptions "emacs";
    package = lib.options.mkPackageOption pkgs "emacs29" { };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.emacs = {
      enable = true;
      package = lib.mkMerge [
        (lib.mkIf cfg.chemacs.enable cfg.chemacs.wrapped)
        (lib.mkIf (!cfg.chemacs.enable) cfg.package)
      ];
    };
