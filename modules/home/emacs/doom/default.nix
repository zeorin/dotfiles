...
let
  ...
  doomemacs = (cfg.package.override { inherit emacs; });
  dpiScale = lib.${namespace}.dpiScale config.${namespace}.dpi;
in
{
  options.${namespace}.emacs.doom = {
  ...
  config = lib.mkIf cfg.enable {
    ...
    "${namespace}".emacs = {
      chemacs = lib.mkIf config.emacs.chemacs.enable {
        profiles = {
          doomemacs = {
    ...
    xdg.configFile = {
      ...
      "doomemacs/config.el".source = pkgs.unstable.replaceVars ./config.el {
        ...
        inherit (config.home.sessionVariables) XDG_DOCUMENTS_DIR XDG_DATA_HOME;
      };
