...
in
{
  ...
  options.${namespace}.common = {
    ...
    nixpkgs.allowUnfreePackages = lib.mkOption {
      type = with lib.types; (listOf (either str (functionTo bool)));
      default = [ ];
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    "${namespace}".emacs = {
      enable = true;
      # TODO chemacs.enable = true;
      doom.enable = true;
      doom.chemacs.default = true;
    };
    ...
    programs = {
      ...
      dircolors = {
        ...
        extraConfig = builtins.readFile "${
          pkgs.${namespace}.nordtheme-dircolors
        }/share/nordtheme/dir_colors";
      };
      direnv = {
        enable = true;
      ...
      firefox = {
        ...
        package = pkgs.firefox.override {
          nativeMessagingHosts = with pkgs; [
            ...
            tridactyl-native
            (pkgs.${namespace}.tabfs.override { mountDir = "${config.xdg.dataHome}/tabfs/personal"; })
          ];
        };
        profiles =
          let
            extensions =
              ...
              ++ (with pkgs; [ pkgs.${namespace}.tabfs ]);
            commonSettings = {
          ...
          in
          {
            personal = {
              ...
              userChrome = ''
                @import url('${
                  pkgs.${namespace}.firefox-csshacks
                }/share/mozilla/firefox/chrome/window_control_placeholder_support.css');
                @import url('${
                  pkgs.${namespace}.firefox-csshacks
                }/share/mozilla/firefox/chrome/hide_tabs_toolbar.css');
                @import url('${
                  pkgs.${namespace}.firefox-csshacks
                }/share/mozilla/firefox/chrome/autohide_toolbox.css');
              '';
              inherit extensions;
            };
            developer-edition = {
      ...
      kitty = {
        ...
        extraConfig = ''
          ...
          # symbol_map ${
            ...
            with lib;
            with strings;
            trivial.pipe (fileContents "${pkgs.unstable.unicode-emoji}/share/unicode/emoji/emoji-sequences.txt")
              [
                (splitString "\n")
        ...
        settings =
          ...
          in
          {
            ...
            "module/audio-input" = {
              ...
              tail = true;
              exec = ''${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --node-type input --icons-volume "󰍬" --icon-muted "󰍭" --color-muted ${lib.strings.removePrefix "#" colors.nord0} --node-blacklist "*.monitor" --notifications listen'';
              click-right = "exec ${pkgs.pavucontrol}/bin/pavucontrol &";
              click-left = "${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --node-type input togmute";
              click-middle = "${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --node-type input next-node";
              scroll-up = "${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --node-type input --volume-max 130 up";
              scroll-down = "${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --node-type input --volume-max 130 down";
            };
            "module/audio-output" = {
              "inherit" = "powerline/right-section-separator";
              format-foreground = colors.nord4;
              ...
              exec = ''${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --icons-volume "󰕿 ,󰖀 ,󰕾 " --icon-muted "󰖁 " --color-muted ${lib.strings.removePrefix "#" colors.nord0} --node-nicknames-from "device.description" --notifications listen'';
              click-right = "exec ${pkgs.pavucontrol}/bin/pavucontrol &";
              click-left = "${pkgs.${namespace}.polybar-pulseaudio-control}/bin/pulseaudio-control togmute";
              click-middle = "${pkgs.${namespace}.polybar-pulseaudio-control}/bin/pulseaudio-control next-node";
              scroll-up = "${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --volume-max 130 up";
              scroll-down = "${
                pkgs.${namespace}.polybar-pulseaudio-control
              }/bin/pulseaudio-control --volume-max 130 down";
            };
            "module/xkeyboard" = {
              "inherit" = "powerline/right-section-separator";
              format-foreground = colors.nord4;
    ...
    xdg = {
      ...
      configFile = with config.xdg; {
        ...
        "readline/inputrc".text = ''
          ...
          $endif
        '';
        "starship.toml".source = pkgs.concatText "starship.toml" [
          (pkgs.fetchurl {
          ...
          (
            ...
            in
            tomlFormat.generate "starship-config" config.programs.starship.settings
          )
        ];
        "todo/config".text = ''
        ...
        "tridactyl/themes".source = pkgs.symlinkJoin {
          ...
          paths = [
            "${pkgs.${namespace}.base16-tridactyl}/share/base16-tridactyl"
      ...
      dataFile = {
        ...
        "fcitx5/themes".source = pkgs.${namespace}.fcitx5-nord;
      };
      portal = {
        enable = true;
    ...
    xresources = {
      ...
      extraConfig = ''
        #include "${pkgs.${namespace}.nordtheme-xresources}/share/nordtheme/xresources"
      '';
    };
    ...
    gtk = {
      ...
      cursorTheme = {
        package = pkgs.${namespace}.colloid-nord-cursor-theme;
    ...
    home.packages =
            ...
            (
              firefox-devedition.override {
                nativeMessagingHosts = with pkgs; [
                  ...
                  plasma-browser-integration
                  tridactyl-native
                  (pkgs.${namespace}.tabfs.override { mountDir = "${config.xdg.dataHome}/tabfs/developer-edition"; })
                ];
              }
            )
          )
            ...
            (
              firefox-beta.override {
                nativeMessagingHosts = with pkgs; [
                  ...
                  plasma-browser-integration
                  tridactyl-native
                  (pkgs.${namespace}.tabfs.override { mountDir = "${config.xdg.dataHome}/tabfs/beta"; })
                ];
              }
            )
          )
            ...
            (
              firefox-esr.override {
                nativeMessagingHosts = with pkgs; [
                  ...
                  plasma-browser-integration
                  tridactyl-native
                  (pkgs.${namespace}.tabfs.override { mountDir = "${config.xdg.dataHome}/tabfs/esr"; })
                ];
              }
            )
          )
      ...
      ++ [
        ...
        prismlauncher
        pkgs.${namespace}.modorganizer2-linux-installer
        manix
        cachix
        nix-prefetch-git
        nix-prefetch
      ...
      ++ [
        ...
        pkgs.${namespace}.nerdfonts-fontconfig

        # Apple Fonts
        pkgs.${namespace}.san-francisco-pro
        pkgs.${namespace}.san-francisco-compact
        pkgs.${namespace}.san-francisco-mono
        pkgs.${namespace}.new-york
