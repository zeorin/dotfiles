  ...
  config = lib.mkIf cfg.enable {
    ...
    home = {
      ...
      activation = with config.xdg; {
        createXdgCacheAndDataDirs = inputs.home-manager.lib.hm.dag.entryAfter [ "writeBoundary" ] ''
          # Cache dirs
        ...
        profiles =
          ...
          in
          {
            personal = {
              ...
              userChrome =
                let
                  chrome = "${pkgs.${namespace}.firefox-csshacks}/share/mozilla/firefox/chrome";
                in
                ''
        ...
        pass = {
          ...
          extraConfig =
            ...
            ''
              ...
              _rofi () {
                  ${pkgs.rofi}/bin/rofi \
                    -dpi ${toString config.${namespace}.dpi} \
                    -i \
                    -kb-accept-custom "" \
      ...
      dunst = {
        ...
        settings = {
          global = {
            ...
            dmenu = "${pkgs.rofi}/bin/rofi -dpi ${toString config.${namespace}.dpi} -dmenu -p dunst";
      ...
      polybar = {
        ...
        settings =
          ...
          in
          {
            ...
            "module/audio-input" =
              let
                pulseaudio-control = "${pkgs.${namespace}.polybar-pulseaudio-control}/bin/pulseaudio-control";
              in
              {
                ...
                type = "custom/script";
                tail = true;
                exec = ''${pulseaudio-control} --node-type input --icons-volume "󰍬" --icon-muted "󰍭" --color-muted ${lib.strings.removePrefix "#" colors.nord0} --node-blacklist "*.monitor" --notifications listen'';
            ...
            "module/audio-output" =
              ...
              in
              {
                ...
                type = "custom/script";
                tail = true;
                exec = ''${pulseaudio-control} --icons-volume "󰕿 ,󰖀 ,󰕾 " --icon-muted "󰖁 " --color-muted ${lib.strings.removePrefix "#" colors.nord0} --node-nicknames-from "device.description" --notifications listen'';
                click-right = "exec ${pkgs.pavucontrol}/bin/pavucontrol &";
      ...
      windowManager.i3 = {
        ...
        config =
          ...
          in
          {
            ...
            modifier = "Mod4";
            terminal = terminal-emulator;
            menu = ''"${pkgs.rofi}/bin/rofi -dpi ${
              toString config.${namespace}.dpi
    ...
    xdg = {
      ...
      configFile = with config.xdg; {
        ...
        "fcitx5".source = pkgs.symlinkJoin {
          ...
          paths = [
            ...
            (pkgs.writeTextDir "conf/classicui.conf" ''
              ...
              UseAccentColor=True
              PerScreenDPI=False
              ForceWaylandDPI=${toString config.${namespace}.dpi}
        ...
        "starship.toml".source =
          ...
          lib.mkIf cfg.enable (
            lib.mkForce (
              pkgs.concatText "starship.toml" [
                (pkgs.runCommandLocal "starship-preset-nerd-font-symbols.toml" { }
        ...
        lib.mkIf cfg.enable (
          lib.mkForce (
            pkgs.concatText ".dir_colors" [
              ...
              "${pkgs.${namespace}.nordtheme-dircolors}/share/nordtheme/dir_colors"
            ]
          )
        );
      ".editrc".text = ''
    ...
    home.packages =
      ...
      ++ [
        ...
        # twitter-color-emoji
        twemoji-color-font
        ttf_bitstream_vera # need by twemoji-color-font
        # noto-fonts-emoji
        ...
        pkgs.${namespace}.nerdfonts-symbols-only
