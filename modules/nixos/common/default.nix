...
let
  ...
  dpiScaleFloat = lib.${namespace}.dpiScaleFloat config.${namespace}.dpi;
in
  ...
  options = {
    ${namespace}.common = {
      enable = lib.options.mkOption {
        default = true;
        example = false;
        description = "Whether to enable common system options";
        type = lib.types.bool;
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    boot = {
      ...
      plymouth = {
        ...
        theme = "breeze";
        extraConfig = ''
          DeviceScale=${toString (dpiScale 1)}
        '';
      };
    ...
    environment.variables = {
      ...
      MOZ_X11_EGL = "1";
      # DPI
      GDK_SCALE = toString (dpiScale 1); # GTK 3 doesn't accept fractional values; also scales fonts
      GDK_DPI_SCALE = toString (1.0 / (dpiScaleFloat 1.0)); # GTK 4 only; accepts fractional values, but ~GDK_SCALE~ scales UI & fonts, but so do the DPI settings, this scales just the fonts back down
      QT_AUTO_SCREEN_SCALE_FACTOR = "1";
      QT_ENABLE_HIGHDPI_SCALING = "1";
      _JAVA_OPTIONS = "-Dsun.java2d.uiScale=${toString (dpiScaleFloat 1.0)}";
    };
    ...
    services.xserver = {
      ...
      screenSection = ''
        # Option "DPI" "${toString config.${namespace}.dpi} x ${toString config.${namespace}.dpi}"
      '';
    ...
    fonts = lib.mkMerge [
      ...
      (lib.mkIf (config.${namespace}.dpi > 200) {
